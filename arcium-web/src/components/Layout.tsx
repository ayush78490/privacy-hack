...
const Layout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    // Android WebView - render normally without glass effect
    if (!isWeb) {
        return (
    ...
    // Web - render with glass card container
    return (
        <>
            ...
            <div className="glass-side-line left hidden lg:block" />
            <div className="glass-side-line right hidden lg:block" />

            {/* Glass card container */}
            <div className="glass-card-container">
                <div className="glass-card-wrapper">
                    <div className={`glass-card-content no-scrollbar ${isAppPage ? 'pb-24' : ''}`}>
                        {children}
                    </div>

                    {isAppPage && (
                        <nav className="absolute bottom-0 left-0 right-0 glass-panel border-t border-white/10 pb-safe pt-2 px-4 z-50 h-[80px] rounded-b-[28px]">
                            <div className="flex justify-around items-center h-full pb-4 text-gray-400">
                                {navItems.map((item) => (
                                    <Link key={item.href} href={item.href}>
                                        <a className={`flex flex-col items-center gap-1 transition-colors group w-16 relative ${location === item.href ? 'text-brand-orange' : 'hover:text-white'}`}>
